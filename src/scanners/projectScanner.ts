...
function calculateScore(
  result: Omit<ProjectScanResult, 'score' | 'warnings' | 'recommendations'>,
  ...
  if (!readmeInfo.exists) {
    ...
    score -= scoring.missingReadmeInstallSection;
  ...
  if (!fileInfo.hasEnvExample && !packageInfo.isCli) {
    score -= scoring.missingEnvExample;
  ...
  if (packageInfo.exists && !packageInfo.scripts.includes('test')) {
    score -= scoring.missingTestScript;
  ...
  if (packageInfo.exists && !packageInfo.scripts.includes('build')) {
    score -= scoring.missingBuildScript;
  ...
  if (fileInfo.largeFiles.length > 0) {
    const deduction = Math.min(fileInfo.largeFiles.length * scoring.largeFileDeductionEach, scoring.largeFileDeductionMax);
    score -= deduction;
...
export async function scanProject(targetPath: string, config: Config): Promise<ProjectScanResult> {
  const absolutePath = resolve(targetPath);

  const [packageInfo, readmeInfo, fileInfo] = await Promise.all([
    scanPackage(absolutePath),
    scanReadme(absolutePath),
    scanFiles(absolutePath, config.scanning),
  ]);

  const base = { scannedAt: new Date().toISOString(), path: absolutePath, packageInfo, readmeInfo, fileInfo };
  const { score, warnings, recommendations } = calculateScore(base, config.scoring);
